...
export function getCustomers(): Customer[] {
  ...
  // Initial mock data
  const initial = [
    { id: '1', name: 'Acme Corp', contact: 'John Doe', stage: 'Negotiation', score: 95, risk: 10, contacts: [{ id: '1', type: 'Email', value: 'john@acme.com'}], intent: 'High' },
    { id: '2', name: 'Global Tech', contact: 'Jane Smith', stage: 'Qualified', score: 78, risk: 25, contacts: [{ id: '2', type: 'Email', value: 'jane@globaltech.com'}, { id: 'w1', type: 'WhatsApp', value: '15550000001'}], intent: 'Medium' },
    { id: '3', name: 'Synergy Ltd', contact: 'Alice Brown', stage: 'Lead', score: 45, risk: 5, contacts: [{ id: '3', type: 'Email', value: 'alice@synergy.com'}], intent: 'Low' },
  ].map(c => ({
    ...c,
    logs: [
      { id: '1', time: 'Today, 10:30 AM', event: 'AI identified high read-rate on Quotation #1044', type: 'ai' },
...
export function getAgentRuns(): AgentRun[] {
  ...
  const initialRuns: AgentRun[] = [
    { id: 'run_1', customerId: '1', agentId: '3', taskType: 'Follow-up Email', status: 'Pending', createdAt: new Date(Date.now() - 3600000).toISOString() }
  ];
...
export function getAgentSteps(): AgentStep[] {
  ...
  const initialSteps: AgentStep[] = [
    { id: 's_1', runId: 'run_1', stepType: 'Tool', toolName: 'get_customer_profile', status: 'Success', createdAt: new Date(Date.now() - 3600000).toISOString() },
    { id: 's_2', runId: 'run_1', stepType: 'Tool', toolName: 'draft_email', status: 'Success', createdAt: new Date(Date.now() - 3590000).toISOString() }
  ];
...
export function getAgentApprovals(): AgentApproval[] {
  ...
  const initialApprovals: AgentApproval[] = [
    { id: 'app_1', runId: 'run_1', actionType: 'send_email', proposedPayload: { to: 'john@acme.com', subject: 'Follow up on our discussion', body: 'Hi John,\n\nJust checking in on the volume discounts we discussed. Let me know if you need any adjustments to the quote.' }, status: 'Pending', createdAt: new Date(Date.now() - 3590000).toISOString() }
  ];
...
export function getAgents(): Agent[] {
  try {
    ...
    if (data) {
       ...
       if (!parsed.find((a: any) => a.id === '5')) {
          parsed.push({ id: '5', name: 'Lead Generation Agent', role: 'Navigates and scrapes leads autonomously.', status: 'Active', tasks: 0, harness: 'Human-in-the-loop', integrations: ['Outscraper', 'Apify', 'PhantomBuster', 'Scrap.io', 'HasData', 'Decodo', 'Clay.com'] });
          saveAgents(parsed);
       }
       return parsed;
    }
  ...
  const initial: Agent[] = [
    ...
    { id: '5', name: 'Lead Generation Agent', role: 'Navigates and scrapes leads autonomously.', status: 'Active', tasks: 0, harness: 'Human-in-the-loop', integrations: ['Outscraper', 'Apify', 'PhantomBuster', 'Scrap.io', 'HasData', 'Decodo', 'Clay.com'] },
  ];
  saveAgents(initial);
  return initial;
}
...
export function addAgent(agent: Omit<Agent, 'id' | 'tasks'>) {
  ...
  agents.push({ ...agent, id: Math.random().toString(36).substr(2, 9), tasks: 0 });
  saveAgents(agents);
}
...
export function updateInboxMessage(id: string, updates: Partial<MessagePreview>) {
...
export function getInboxMessages(): MessagePreview[] {
  ...
  const initial: MessagePreview[] = [
    { 
      ...
      thread: [
         { id: 't1', sender: 'user', content: 'Hello, I am interested in ordering 10,000 units of the premium widgets. What is the best volume discount you can offer for this quantity? Also, let me know the estimated lead time to North America.', time: '10:30 AM' }
      ]
    },
    { 
      ...
      thread: [
         { id: 't1', sender: 'user', content: 'Hi, I saw your MOQ is 500 units. Since this is our first time working together, is it possible to order 100 units just for quality testing?', time: 'Yesterday 2:15 PM' },
         { id: 't2', sender: 'agent', content: 'Hi there! We strictly follow the MOQ of 500 units for standard pricing, but for a one-time paid sample order, we could do 100 units at a 20% premium. Would that work for you?', time: 'Yesterday 2:40 PM' }
      ]
    }
  ];
...
export function addDraftToThread(messageId: string, reply: string) {
  ...
  if (m) {
    m.thread.push({ id: Math.random().toString(), sender: 'agent', content: reply, time: new Date().toLocaleTimeString() });
    saveInboxMessages(msgs);
  }
}
