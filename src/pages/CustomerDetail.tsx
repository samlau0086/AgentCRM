...
import { ArrowLeft, Clock, History, Bot, Sparkles, Building2, UserCircle, Briefcase, Mail, CheckCircle2, XCircle, Loader2, Send, Edit2 } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useLanguage } from '../i18n';
import { getCustomer, updateCustomer, Customer, CustomerLog, UniversalComment, Attachment, getCurrentUser } from '../services/db';
...
export default function CustomerDetail() {
  ...
  const [draftContent, setDraftContent] = useState('Email Draft: MOQ Negotiation response.');
  ...
  const handleGenerateProposal = () => {
    ...
    setTimeout(() => {
      ...
      if (customer && id) {
        const newLog: CustomerLog = { id: `t_${Date.now()}`, time: 'Just now', event: 'AI drafted a new 5% discount proposal', type: 'ai' };
  ...
  const handleApproveSend = () => {
    ...
    setTimeout(() => {
      ...
      if (customer && id) {
        const newLog: CustomerLog = { id: `t_${Date.now()}`, time: 'Just now', event: `Approved and sent: ${draftContent}`, type: 'comm' };
        const newLogs = [newLog, ...(customer.logs || [])];
        updateCustomer(id, { logs: newLogs });
        setCustomer(getCustomer(id));
      }
  ...
  return (
    <div className="w-full p-8 flex flex-col gap-8">
      ...
      <div className="flex items-center gap-4">
        <Link to="/customers" className="p-2 hover:bg-slate-200 dark:hover:bg-white/10 rounded-lg transition-colors border border-transparent hover:border-slate-200 dark:border-white/5">
        ...
        {customer ? (
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-semibold text-slate-900 dark:text-white tracking-tight">{customer.name}</h1>
              <span className="px-2 py-1 bg-purple-900/40 text-purple-400 border border-purple-500/20 rounded text-[10px] font-mono">{customer.stage}</span>
              {(customer.tags || []).map((tag, idx) => (
                <span key={idx} className="px-2 py-1 bg-blue-50 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-500/20 rounded text-[10px] font-mono">
                  {tag}
                </span>
              ))}
            </div>
            <p className="text-slate-400 dark:text-slate-500 dark:text-slate-400 mt-1 flex flex-wrap items-center gap-2 text-sm font-light">
              ...
              {customer.contacts?.map((c, i) => (
                <span key={i} className="flex items-center gap-1 ml-2 border-l border-slate-300 dark:border-white/10 pl-2">
                  {c.type === 'Email' ? <Mail className="w-4 h-4 shrink-0" /> : <Briefcase className="w-4 h-4 shrink-0"/>}
                  <span>{c.value}</span>
                </span>
              ))}
            </p>
      ...
      {customer && (
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-gradient-to-br from-blue-900/20 to-black/20 border border-blue-600/20 dark:border-blue-500/20 rounded-2xl p-6 relative overflow-hidden">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <h2 className="text-xs font-semibold uppercase tracking-widest text-blue-600 dark:text-blue-400">{t('cd.aiSummary')}</h2>
            </div>
            <p className="text-slate-700 dark:text-slate-300 text-sm leading-relaxed font-light">
              Customer is highly interested in bulk ordering 10,000 units but is currently negotiating on the Minimum Order Quantity (MOQ) criteria. They have opened the last pricing proposal 3 times in the past 24 hours. The overall intent is very high, but risk of stalling exists if MOQ flexibility is not addressed.
            </p>
          </div>
          ...
          {showInsight && (
            <div className="bg-white dark:bg-white/5 shadow-sm dark:shadow-none border border-emerald-600/30 dark:border-emerald-500/30 rounded-2xl p-6 relative overflow-hidden">
              ...
              <h2 className="text-[10px] font-bold text-emerald-600 dark:text-emerald-500 mb-2 uppercase tracking-widest">{t('cd.nextAction')}</h2>
              <p className="text-slate-900 dark:text-white text-lg font-light mb-6">
                Suggest offering a <span className="font-semibold">5% discount</span> or splitting the MOQ across two deliveries to close the deal today.
              </p>
          ...
          <div className="bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest">{t('cd.timeline')}</h2>
              <button 
              ...
              {(customer?.logs || []).slice(0, 3).map((item, i) => (
                <div key={item.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-slate-200 dark:border-white/10 bg-black text-slate-400 dark:text-slate-500 group-[.is-active]:border-blue-500/30 group-[.is-active]:text-blue-600 dark:text-blue-400 shadow-xl shrink-0 md:order-1 md:group-odd:-translate-x-1/2 md:group-even:translate-x-1/2 z-10">
                    {item.type === 'ai' ? <Bot className="w-4 h-4" /> : item.type === 'comm' ? <Mail className="w-4 h-4" /> : <History className="w-4 h-4" />}
                  </div>
                  <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded-xl border border-slate-200 dark:border-white/5 bg-white dark:bg-white/5 shadow-sm dark:shadow-none hover:bg-white/[0.08] transition-colors">
                    <div className="flex items-center justify-between mb-1">
                      <div className="font-medium text-slate-800 dark:text-slate-200 text-sm text-center">{item.event}</div>
                    </div>
                    <div className="text-xs text-slate-400 dark:text-slate-500 font-mono">{item.time}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-6">
                ...
                <div className="p-4 bg-amber-950/20 border border-amber-600/20 dark:border-amber-500/20 rounded-xl relative overflow-hidden">
                  {isEditingDraft ? (
                    ...
                    <div className="flex flex-col gap-4 relative z-10">
                      <p className="text-sm text-amber-600 dark:text-amber-500 font-light">{draftContent}</p>
          ...
          {/* Profile Memory */}
          <div className="bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl p-6">
            <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-4">{t('cd.profileMemory')}</h2>
            <div className="space-y-4">
              <div className="flex gap-3 text-sm">
                <Building2 className="w-5 h-5 text-slate-600 shrink-0" />
                <div>
                  <p className="text-slate-400 dark:text-slate-500 text-[10px] uppercase tracking-widest">{t('cd.indSize')}</p>
                  <p className="text-slate-800 dark:text-slate-200 font-mono mt-1 text-xs">{customer.industry || 'Manufacturing, 100-500 emp.'}</p>
                </div>
              </div>
              <div className="flex gap-3 text-sm mt-4 border-t border-slate-200 dark:border-white/5 pt-4">
                ...
                <div>
                  <p className="text-slate-400 dark:text-slate-500 text-[10px] uppercase tracking-widest">{t('cd.budget')}</p>
                  <p className="text-slate-800 dark:text-slate-200 font-mono mt-1 text-xs">$50k - $100k</p>
                </div>
              </div>
              ...
              {customer.address || customer.city || customer.province ? (
                <div className="flex gap-3 text-sm mt-4 border-t border-slate-200 dark:border-white/5 pt-4">
                  ...
                  <div>
                    <p className="text-slate-400 dark:text-slate-500 text-[10px] uppercase tracking-widest">Office Address</p>
              ...
              {customer.description && (
                <div className="flex gap-3 text-sm mt-4 border-t border-slate-200 dark:border-white/5 pt-4">
                  <div className="w-5 h-5 flex items-center justify-center shrink-0">ℹ️</div>
                  <div>
                    <p className="text-slate-400 dark:text-slate-500 text-[10px] uppercase tracking-widest">Description</p>
          ...
          <div className="bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-2xl p-6">
            <h2 className="text-xs font-semibold text-slate-400 dark:text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-4">{t('cd.semanticMemory')}</h2>
            <div className="flex flex-wrap gap-2">
              <span className="px-2.5 py-1 bg-white dark:bg-white/5 shadow-sm dark:shadow-none border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 text-[10px] font-mono rounded">Strict on delivery times</span>
              <span className="px-2.5 py-1 bg-white dark:bg-white/5 shadow-sm dark:shadow-none border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 text-[10px] font-mono rounded">Prefers WhatsApp</span>
              <span className="px-2.5 py-1 bg-white dark:bg-white/5 shadow-sm dark:shadow-none border border-slate-200 dark:border-white/10 text-slate-700 dark:text-slate-300 text-[10px] font-mono rounded">Requires PO process</span>
            </div>
          </div>
        </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[80vh] border border-slate-200 dark:border-white/10 animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
              <h3 className="font-semibold text-slate-900 dark:text-white">Complete Activity Log</h3>
              <button 
            ...
            <div className="overflow-y-auto p-6 space-y-6 relative before:absolute before:inset-0 before:ml-11 before:-translate-x-px before:h-full before:w-px before:bg-slate-200 dark:before:bg-white/10">
              ...
              {(customer?.logs || []).map((item, i) => (
                <div key={item.id} className="relative flex items-center gap-6 group is-active">
                  <div className="flex items-center justify-center w-10 h-10 rounded-full border border-slate-200 dark:border-white/10 bg-black text-slate-400 dark:text-slate-500 shadow-sm shrink-0 z-10 bg-white dark:bg-slate-900">
                    {item.type === 'ai' ? <Bot className="w-4 h-4 text-blue-500" /> : item.type === 'comm' ? <Mail className="w-4 h-4 text-emerald-500" /> : <History className="w-4 h-4 text-amber-500" />}
                  </div>
                  <div className="flex-1 p-4 rounded-xl border border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/5 shadow-sm">
                    <div className="font-medium text-slate-800 dark:text-slate-200 text-sm mb-1">{item.event}</div>
                    <div className="text-xs text-slate-400 dark:text-slate-500 font-mono">{item.time}</div>
                  </div>
                </div>
              ))}
